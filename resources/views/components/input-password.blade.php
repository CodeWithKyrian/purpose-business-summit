@props(['disabled' => false, 'name' => 'password'])

<div class="mb-4" x-data="{ view: false }">
    <div class="flex w-full items-center border border-gray-400 rounded-md focus:border-blue-400 overflow-hidden p-2">
        ...
        <input name="{{$name}}" {!! $attributes->merge(['class' => 'bg-transparent w-full text-sm focus:ring-0 outline-none focus:outline-none focus:border-0 border-0']) !!} :type="view ? 'text' : 'password'" required />
        @error('password')
            <svg class="h-6 w-6 m-2 fill-current text-red-600" viewBox="0 0 295.43 295.43"
                xmlns="http://www.w3.org/2000/svg">
                <path
        ...
        <div class="cursor-pointer" @click="view = !view">
            <svg x-show="view" x-cloak class="w-6 h-6 text-blue-600" viewBox="0 0 24 24" fill="none"
                xmlns="http://www.w3.org/2000/svg">
